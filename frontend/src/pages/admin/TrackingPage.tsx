...
export default function TrackingPage() {
  ...
  const friendlyFieldNames: Record<string, string> = {
    ...
    RelatedAppointmentId: 'Mã lịch hẹn liên quan',
    balance: 'Số dư',
    currency: 'Loại tiền tệ',
    walletId: 'Mã ví',
    amount: 'Số tiền',
    ...
    CommissionDeductions: 'Khấu trừ hoa hồng',
    NetSalary: 'Lương ròng',
    PaidAt: 'Thanh toán vào',
    periodStartDate: 'Ngày bắt đầu kỳ',
    periodEndDate: 'Ngày kết thúc kỳ',
    totalAppointments: 'Tổng lịch hẹn',
    totalEarnings: 'Tổng thu nhập',
    ...
    NormalizedUserName: 'Tên đăng nhập chuẩn hóa',
    NormalizedEmail: 'Email chuẩn hóa',
    EmailConfirmed: 'Email đã xác nhận',
    PhoneNumberConfirmed: 'Số điện thoại đã xác nhận',
    AccessFailedCount: 'Số lần truy cập thất bại',
    IsProfileCompleted: 'Hồ sơ đã hoàn thành',
    LockoutEnd: 'Kết thúc khóa tài khoản',
    IsTemporaryUsername: 'Tên đăng nhập tạm thời',
    userName: 'Tên đăng nhập',
    normalizedUserName: 'Tên đăng nhập chuẩn hóa',
    normalizedEmail: 'Email chuẩn hóa',
    emailConfirmed: 'Email đã xác nhận',
    ...
    ReminderTypeCode: 'Loại nhắc nhở',
    ScheduledDate: 'Ngày lên lịch',
    IsRecurring: 'Lặp lại',
    RecurrencePattern: 'Mẫu lặp lại',
    IsCompleted: 'Đã hoàn thành',
    CompletedAt: 'Hoàn thành vào',
    patientId: 'Mã bệnh nhân',
    reminderTypeCode: 'Loại nhắc nhở',
    title: 'Tiêu đề',
    scheduledDate: 'Ngày lên lịch',
    ...
    isCompleted: 'Đã hoàn thành',
    completedAt: 'Hoàn thành vào',
    ToBin: 'Mã ngân hàng đích',
    ToAccountNumber: 'Số tài khoản đích',
    FromBin: 'Mã ngân hàng nguồn',
    FromAccountNumber: 'Số tài khoản nguồn',
    ...
    GenericName: 'Tên thuốc học',
    DosageForms: 'Dạng liều',
    CommonUses: 'Công dụng phổ biến',
    ...
    commonUses: 'Công dụng phổ biến',
    sideEffects: 'Tác dụng phụ',
    ServiceTierId: 'Mã gói dịch vụ',
    Name: 'Tên',
    ConsultationFeeMultiplier: 'Hệ số phí tư vấn',
    PriorityBoost: 'Tăng ưu tiên',
    MaxDailyAppointments: 'Số lịch hẹn tối đa/ngày',
    Features: 'Tính năng',
    ...
    SpecializationCode: 'Mã chuyên khoa',
    specialization: 'Chuyên khoa',
    specializationCode: 'Mã chuyên khoa',
  };
  ...
  const getFriendlyValue = (key: string, value: any): string => {
    ...
    if (typeof value === 'boolean') {
      ...
      if (keyForCheck === 'IsActive') return value ? 'Hoạt động' : 'Không hoạt động';
      if (keyForCheck === 'IsUsed') return value ? 'Đã sử dụng' : 'Chưa sử dụng';
      if (keyForCheck === 'EmailConfirmed') return value ? 'Đã xác nhận' : 'Chưa xác nhận';
      if (keyForCheck === 'PhoneNumberConfirmed') return value ? 'Đã xác nhận' : 'Chưa xác nhận';
      if (keyForCheck === 'IsProfileCompleted') return value ? 'Đã hoàn thành' : 'Chưa hoàn thành';
      if (keyForCheck === 'IsRecurring') return value ? 'Có lặp lại' : 'Không lặp lại';
      if (keyForCheck === 'IsCompleted') return value ? 'Đã hoàn thành' : 'Chưa hoàn thành';
      if (keyForCheck === 'IsTemporaryUsername') return value ? 'Tạm thời' : 'Chính thức';
  ...
  const entityLabelMap: Record<string, string> = {
    ...
    DoctorScheduleOverride: 'Lịch linh hoạt của bác sĩ',
    Wallet: 'Ví tiền',
    WalletTransaction: 'Giao dịch ví tiền',
    EmailVerificationCode: 'Mã xác minh email',
    DoctorSalary: 'Lương bác sĩ',
    User: 'Người dùng',
    PatientHealthReminder: 'Nhắc nhở sức khỏe bệnh nhân',
    TransferTransaction: 'Giao dịch chuyển tiền',
    DoctorRegistrationForm: 'Biểu mẫu đăng ký bác sĩ',
    ServiceTierSubscription: 'Gói dịch vụ',
    Prescription: 'Đơn thuốc',
    MedicationDatabase: 'Cơ sở dữ liệu thuốc',
    DoctorServiceTier: 'Gói dịch vụ bác sĩ',
    Specialization: 'Chuyên khoa',
  };
