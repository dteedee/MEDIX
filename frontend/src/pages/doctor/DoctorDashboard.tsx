...
const DoctorDashboard: React.FC = () => {
  ...
  const todaySchedule: TodayScheduleSlot[] = React.useMemo(() => {
    ...
    const visibleFixedSlots: TodayScheduleSlot[] = regular.filter(fixedSlot =>
      !overrides.some(overrideSlot =>
    ...
    return [...visibleFixedSlots, ...visibleOverrideSlots]
        ...
        <div className={styles.statsGrid}>
          ...
          <>
            <div className={`${styles.statCard} ${styles.statCard1}`}>
              ...
              <div className={styles.statBg}><i className="bi bi-cash-coin"></i></div>
            </div>

            <div className={`${styles.statCard} ${styles.statCard2}`}>
              <div className={styles.statIcon}><i className="bi bi-wallet2"></i></div>
              <div className={styles.statContent}>
              ...
              <div className={styles.statBg}><i className="bi bi-wallet2"></i></div>
            </div>

            <div className={`${styles.statCard} ${styles.statCard5}`}>
              <div className={styles.statIcon}><i className="bi bi-gem"></i></div>
              <div className={styles.statContent}>
              ...
              <div className={styles.statBg}><i className="bi bi-gem"></i></div>
            </div>

            <div className={`${styles.statCard} ${styles.statCard3}`}>
              <div className={styles.statIcon}><i className="bi bi-calendar-check"></i></div>
              <div className={styles.statContent}>
              ...
              <div className={styles.statBg}><i className="bi bi-calendar-check"></i></div>
            </div>

            {dashboardData.wallet && (
              <div className={`${styles.statCard} ${styles.statCard6}`}>
                <div className={styles.statIcon}><i className="bi bi-wallet-fill"></i></div>
                <div className={styles.statContent}>
                ...
                <div className={styles.statBg}><i className="bi bi-wallet-fill"></i></div>
              </div>
            )}


            <div className={`${styles.statCard} ${styles.statCard4}`}>
              <div className={styles.statIcon}><i className="bi bi-star-half"></i></div>
              <div className={styles.statContent}>
              ...
              <div className={styles.statBg}><i className="bi bi-star-half"></i></div>
            </div>
          </>
          ) : (
      ...
      <div className={styles.contentGrid}>
                    ...
                    <div className={styles.reviewMeta}>
                      <div className={styles.stars}>
                        {Array.from({ length: 5 }, (_, i) => (
                          <i key={i} className={`bi bi-star-fill ${i < review.rating ? styles.starFilled : styles.starEmpty}`}></i>
                        ))}
                      </div>
        ...
        <div className={`${styles.card} ${styles.todayScheduleCard}`}>
          <div className={styles.cardHeader}>
            ...
            <Link to="/app/doctor/schedule" className={styles.viewAllBtn}>Quản lý</Link>
          </div>
              ...
              todaySchedule.map((slot, index) => {
                ...
                return (
                  <div key={index} className={styles.scheduleItem}>
                    <span className={styles.scheduleTime}>{formatTime(slot.startTime)} - {formatTime(slot.endTime)}</span>
                    <span className={`${styles.scheduleStatus} ${statusClass}`}>{statusText}</span>
                    {slot.type === 'override' && slot.reason && <span className={styles.scheduleReason}>({slot.reason})</span>}
                  </div>
                );
              })
            ) : (
        ...
        <div className={styles.otherInfoGrid}>
          {!isDashboardLoading && dashboardData?.subscription && (
            <div className={`${styles.card} ${styles.infoCard}`}>
              ...
              <div className={styles.infoCardContent}>
                <div className={`${styles.infoBadge} ${styles.subscriptionBadge}`}>{dashboardData.subscription.name}</div>
                <p className={styles.infoText}>Phí: {formatCurrency(dashboardData.subscription.monthlyFee)}/tháng</p>
                <Link to="/app/doctor/packages" className={styles.infoLink}>Nâng cấp</Link>
              </div>
            </div>
          )}
          {!isDashboardLoading && dashboardData?.salary && (
            <div className={`${styles.card} ${styles.infoCard}`}>
              ...
              <div className={styles.infoCardContent}>
                <div className={`${styles.infoBadge} ${dashboardData.salary.status === 'Paid' ? styles.salaryPaid : styles.salaryPending}`}>
                  {dashboardData.salary.status}
                </div>
                <p className={styles.infoText}>{formatCurrency(dashboardData.salary.netSalary)}</p>
                <span className={styles.infoSubtext}>Kỳ: {new Date(dashboardData.salary.periodStartDate).toLocaleDateString('vi-VN')} - {new Date(dashboardData.salary.periodEndDate).toLocaleDateString('vi-VN')}</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className={`${styles.card} ${styles.quickActionsCard}`}>
          ...
          <div className={styles.quickActionsGrid}>
            <Link to="/app/doctor/appointments" className={styles.actionItem}>
              <i className="bi bi-calendar-week"></i><span>Lịch hẹn</span>
            </Link>
            <Link to="/app/doctor/patients" className={styles.actionItem}>
              <i className="bi bi-people"></i><span>Bệnh nhân</span>
            </Link>
            <Link to="/app/doctor/wallet" className={styles.actionItem}>
              <i className="bi bi-wallet"></i><span>Doanh thu</span>
            </Link>
            <Link to="/app/doctor/profile" className={styles.actionItem}>
              <i className="bi bi-person-circle"></i><span>Hồ sơ</span>
            </Link>
          </div>
        </div>
