...
import { ApiResponse, BloodTypeDTO, FormData, PatientDTO, RegisterDTO, RegistrationPayload } from '../types/registrationTypes';
...
export const patientRegistrationApiService = {
  ...
  registerPatient: async (formData: FormData): Promise<ApiResponse> => {
    try {
      ...
      console.log('API URL:', buildApiUrl('api/register/register'));

      const response = await fetch(buildApiUrl('/api/register/register'), {
        method: 'POST',
        headers: {
